...
define( 'AS_GBPRIMEPAY_VERSION', '2.3.0' );
...
if (!class_exists('AS_Gbprimepay')) {
    class AS_Gbprimepay
    {
        ...
        public function create_gbprimepay_qrcode_post_type() {
					...
					if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_qrcode') {
								...
								remove_action('init', 'create_gbprimepay_qrcode_post_type');
								flush_rewrite_rules();
							}
						}
        }
        public function modify_gbprimepay_qrcode_post_type( $post_type, $args ) {
					global  $wp, $wp_post_types, $woocommerce;
					$script_name = $_SERVER['SCRIPT_NAME'];	
					if(strpos($script_name, 'ajax') !== false){
						return true;
					}
					if(strpos($post_type, 'gbprimepay_') !== false){
					if ( 'gbprimepay_qrcode' === $post_type ) {
						if( isset( WC()->session ) ){
						$chosen_payment_method = WC()->session->get('chosen_payment_method');
					if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_qrcode') {
							$args->label = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
							$args->labels->name = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
							$args->labels->singular_name = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
							$args->labels->all_items = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
							$args->labels->archives = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
							$args->labels->menu_name = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
							$args->labels->name_admin_bar = __( 'GBPrimePay QR Code', 'gbprimepay_qrcode' );
        ...
        public function create_gbprimepay_qrcredit_post_type() {
					...
					if( isset( WC()->session ) ){
					$chosen_payment_method = WC()->session->get('chosen_payment_method');
					if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_qrcredit') {
								...
								remove_action('init', 'create_gbprimepay_qrcredit_post_type');
								flush_rewrite_rules();
							}
						}
        }
        public function modify_gbprimepay_qrcredit_post_type( $post_type, $args ) {
					global  $wp, $wp_post_types, $woocommerce;
					$script_name = $_SERVER['SCRIPT_NAME'];	
					if(strpos($script_name, 'ajax') !== false){
						return true;
					...
					if(strpos($post_type, 'gbprimepay_') !== false){
					if ( 'gbprimepay_qrcredit' === $post_type ) {
						if( isset( WC()->session ) ){
						$chosen_payment_method = WC()->session->get('chosen_payment_method');
					if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_qrcredit') {
							$args->label = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$args->labels->name = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$args->labels->singular_name = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$args->labels->all_items = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$args->labels->archives = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$args->labels->menu_name = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$args->labels->name_admin_bar = __( 'GBPrimePay QR Visa', 'gbprimepay_qrcredit' );
							$wp_post_types[ $post_type ] = $args;
					}else{
						return true;
					}
        ...
        public function create_gbprimepay_qrwechat_post_type() {
				...
				if( isset( WC()->session ) ){
				$chosen_payment_method = WC()->session->get('chosen_payment_method');
				if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_qrwechat') {
							...
							remove_action('init', 'create_gbprimepay_qrwechat_post_type');
							flush_rewrite_rules();
						}
					}
			}
        public function modify_gbprimepay_qrwechat_post_type( $post_type, $args ) {
					global  $wp, $wp_post_types, $woocommerce;
					$script_name = $_SERVER['SCRIPT_NAME'];	
					if(strpos($script_name, 'ajax') !== false){
						return true;
					...
					if(strpos($post_type, 'gbprimepay_') !== false){
					if ( 'gbprimepay_qrwechat' === $post_type ) {
						if( isset( WC()->session ) ){
						$chosen_payment_method = WC()->session->get('chosen_payment_method');
					if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_qrwechat') {
							$args->label = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$args->labels->name = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$args->labels->singular_name = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$args->labels->all_items = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$args->labels->archives = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$args->labels->menu_name = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$args->labels->name_admin_bar = __( 'GBPrimePay QR Wechat', 'gbprimepay_qrwechat' );
							$wp_post_types[ $post_type ] = $args;
					}else{
						return true;
					}
        ...
        public function create_gbprimepay_barcode_post_type() {
				...
				if( isset( WC()->session ) ){
				$chosen_payment_method = WC()->session->get('chosen_payment_method');
				if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_barcode') {
							...
							remove_action('init', 'create_gbprimepay_barcode_post_type');
							flush_rewrite_rules();
						}
					}
			}
        public function modify_gbprimepay_barcode_post_type( $post_type, $args ) {
					global  $wp, $wp_post_types, $woocommerce;
					$script_name = $_SERVER['SCRIPT_NAME'];	
					if(strpos($script_name, 'ajax') !== false){
						return true;
					...
					if(strpos($post_type, 'gbprimepay_') !== false){
					if ( 'gbprimepay_barcode' === $post_type ) {
						if( isset( WC()->session ) ){
						$chosen_payment_method = WC()->session->get('chosen_payment_method');
					if ($chosen_payment_method && $chosen_payment_method === 'gbprimepay_barcode') {
							$args->label = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$args->labels->name = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$args->labels->singular_name = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$args->labels->all_items = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$args->labels->archives = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$args->labels->menu_name = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$args->labels->name_admin_bar = __( 'GBPrimePay Bill Payment', 'gbprimepay_barcode' );
							$wp_post_types[ $post_type ] = $args;
					}else{
						return true;
					}
       ...
       return admin_url( 'admin.php?page=wc-settings&tab=gbprimepay_settings');
        ...
        public function woocommerce_payment_token_deleted($token_id, $token)
        {
            if ( 'gbprimepay' === $token->get_gateway_id() ) {
				...
				add_action( 'registered_post_type', array( $this, 'modify_gbprimepay_qrcode_post_type' ), 20, 2 );
				...
				add_action( 'registered_post_type', array( $this, 'modify_gbprimepay_qrcredit_post_type' ), 20, 2 );
				...
				add_action( 'registered_post_type', array( $this, 'modify_gbprimepay_qrwechat_post_type' ), 20, 2 );
				...
				add_action( 'registered_post_type', array( $this, 'modify_gbprimepay_barcode_post_type' ), 20, 2 );
